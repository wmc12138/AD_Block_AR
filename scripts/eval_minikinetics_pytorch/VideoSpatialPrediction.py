...
def VideoSpatialPrediction(
    ...
    _, _, _, c = rgb.shape  #224*224*3*250  250=10*25
    rgb_list = []
    ...
    # print(rgb_np.shape)  #250*3*224*224
    batch_size = 64
    prediction = np.zeros((num_categories,rgb.shape[3])) #101*250
    num_batches = int(math.ceil(float(rgb.shape[3])/batch_size)) #250/8  = 32

    for bb in range(num_batches):
        span = range(batch_size*bb, min(rgb.shape[3],batch_size*(bb+1)))  #每个span都为8的范围，相当于batch size
        ...
        prediction[:, span] = np.transpose(result)  #一个视频选25帧，每帧再通过数据增强剪切出10帧。总共250幅图像， 250*3*224*224 ，以8为batch_size进行预测。每个batch输出是8*101，依次放入预先准备好的101*250的数组中。
