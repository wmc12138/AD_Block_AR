...
def change_key_names(old_params, in_channels):
    ...
    for layer_key in allKeyList:
        if layer_count >= len(allKeyList)-2:
            # exclude fc layers,即'fc_action.weight', 'fc_action.bias'
            continue
        else:
            if layer_count == 0:
                rgb_weight = old_params[layer_key]        #torch.Size([64, 3, 7, 7])
                # print(type(rgb_weight))
                rgb_weight_mean = torch.mean(rgb_weight, dim=1) #torch.Size([64, 7, 7])
                # TODO: ugly fix here, why torch.mean() turn tensor to Variable
                # print(type(rgb_weight_mean))
                flow_weight = rgb_weight_mean.unsqueeze(1).repeat(1,in_channels,1,1) #torch.Size([64, 1, 7, 7]) -> torch.Size([64, 20, 7, 7]) 
...
if __name__ == "__main__":
    model = flow_resnet50(pretrained=True, num_classes=101)
    state_dict = model.state_dict()
    print('-'*50)
    # c=model.modules().__next__()
    # d=model.modules().__next__()
    # e=model.modules().__next__()
    # print(c)
    # print('*'*100)
    # print(d)
    # print(e)
